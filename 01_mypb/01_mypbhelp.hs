...
p1 : オブジェクトハンドル
%inst
この命令は、プログレスバーにマーキー･アニメーションのスタイル付与・解除を行なうものです。
^
p1にはオブジェクトハンドルを指定して下さい。
^
この命令が呼び出された時点でのプログレスバーのスタイルが通常のスタイル(0)であればマーキー(2)に、垂直スタイル(1)であれば垂直マーキー(3)に変更します。
また、それぞれ逆方向への変更も行いますが、通常スタイル(0)及び垂直スタイル(1)間の変更は行いません。
^
    通常スタイル(0) <---> マーキー(2)
    垂直スタイル(1) <---> 垂直マーキー(3)
^
また、各スタイルの値はmypb命令と同様のものです。
^
    値 |   見た目
----------------------------------------
    0  : 水平のプログレスバー
    ...
    3  : 垂直マーキー
^
この命令によってマーキー(2)もしくは垂直マーキー(3)に変更されると、自動でpbswmarq命令による再生とpbsetst命令による状態の変更が行われます。
%sample
...
-------------------------------------------------
    ...
    3  :  PBST_PAUSED (一時停止)  :  黄色
^
この命令は、pbsetst命令の項目で示しているサンプルスクリプトをそのまま命令化したものです。
...
^
p1にオブジェクトハンドルを指定して下さい。
p2に取得したい情報の取得タイプを指定して下さい。戻り値はシステム変数statに格納されます。
^
タイプ |            内容          |         返り値（stat）
------------------------------------------------------------------------------------
    ...
    6  :  スタイル                :  mypb命令またはpbcstyle命令で設定された整数値
^
取得タイプ4の返り値は次のいずれかになります。
返り値 |     内容
-------------------------------
    1  :  PBST_NORMAL (緑色)
    2  :  PBST_ERROR  (赤色)
...
-----------------------
    ...
    3  :  垂直マーキー
^
※注意事項として、pbinfo関数は "関数"ですので必ずパラメータを丸括弧で囲んで下さい。
...
p1 : オブジェクトハンドル
%inst
この関数は、pbinfo関数のスタイルを返す関数形式のマクロです。
p1にはオブジェクトハンドルを指定して下さい。
^
pb_sty(p1) と pbinfo(p1, 6) は全く同じ値を返します。
...
*main
    wait 16
    pbstep hProg(0)            ; ステップ進行
    pbstcol2 hProg(0), 25, 45  ; 現在位置を取得し、その値に応じて状態を変更
